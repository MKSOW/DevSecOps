...
1
Code source forké
Avec tous vos ajouts (Dockerfile, tests, workflows)
2
RAPPORT.md
Captures + réponses aux questions de chaque étape
3
URL Azure publique
Lien vers votre app déployée et fonctionnelle
4
Workflow GitHub Actions vert
Tous les jobs passent
5
Rapport k6
Fichier k6-summary.json ou capture du résumé
Mise en route
...
Comptes nécessaires
GitHub
Azure for Students : activez votre compte sur azure.microsoft.com/free/students — vous obtenez 100$ de crédit + services gratuits sans carte bancaire.
Cloner et démarrer
...
npm run dev
​
Ouvrez http://localhost:3000, connectez-vous avec admin@helpdesk.io / Password123! et explorez l'app avant de continuer.
...
Allez sur jwt.io, collez le token, et tentez de modifier le rôle dans le payload ("role": "ADMIN"). Pour resigner, utilisez le secret du .env.
...
Poussez votre code sur GitHub : git push
Onglet Actions → vérifiez que les jobs test, security, docker passent (pas deploy pour l'instant).
Si un job échoue, corrigez avant de continuer.
...
Valeur
AZURE_CREDENTIALS
sortie de az ad sp create-for-rbac --sdk-auth
ACR_LOGIN_SERVER
$ACR_SERVER
ACR_USERNAME
$ACR_USER
...
Architecture finale : un schéma simple (ASCII ou Excalidraw) montrant le flux complet dev → GitHub → CI → ACR → App Service.
3 améliorations DevSecOps que vous mettriez en place avec plus de temps (ex: Key Vault pour les secrets, monitoring App Insights, scan SAST avec SonarQube, etc.).
Coût Azure estimé : combien votre crédit étudiant a-t-il été entamé ?
Ce qui vous a posé problème et comment vous l'avez résolu.
